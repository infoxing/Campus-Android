...
class EduroamFixCard(context: Context) :
    ...
    private val isConfigValid: Boolean
        get() {
            ...
            // Check attributes - check newer match for the radius server for all configurations
            ...
            if (eapMethod == Eap.TTLS && (phase2 == Phase2.MSCHAPV2 || phase2 == Phase2.PAP)
                    || eapMethod == Eap.PEAP && phase2 == Phase2.MSCHAPV2) {
                checkDNSName()
    ...
    override fun shouldShow(prefs: SharedPreferences): Boolean {
        ...
        return if (!context.applicationContext.wifiManager.isWifiEnabled) false
        else !isConfigValid
    ...
    private fun checkDNSName() {
        if (SDK_INT < M && !isValidSubjectMatchAPI18()) {
            errors.add(context.getString(R.string.wifi_dns_name_not_set))
        } else if (SDK_INT >= M
                && (eduroam.enterpriseConfig.altSubjectMatch != "DNS:$RADIUS_DNS"
                        || eduroam.enterpriseConfig.domainSuffixMatch != RADIUS_DNS)
                && !isValidSubjectMatchAPI18()) {
            errors.add(context.getString(R.string.wifi_dns_name_not_set))
        }
    }
    ...
    @Suppress("DEPRECATION") // AltSubjectMatch is not available for API18
    private fun isValidSubjectMatchAPI18(): Boolean {
    ...
    companion object : CardAdapter.CardViewHolderFactory {
        ...
        override fun inflateViewHolder(parent: ViewGroup, viewType: Int): CardViewHolder {
            ...
            return EduroamFixCardViewHolder(view)
        }
    }
}
