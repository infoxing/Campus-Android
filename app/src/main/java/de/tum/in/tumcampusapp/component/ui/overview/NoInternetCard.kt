...
class NoInternetCard(context: Context) : StickyCard(CardManager.CARD_NO_INTERNET, context) {
    ...
    override fun updateViewHolder(viewHolder: RecyclerView.ViewHolder) {
        ...
        val lastUpdate = viewHolder.itemView.findViewById<TextView>(R.id.card_last_update)
        val lastUpdated = DateTime(DownloadService.lastUpdate(context))
        val time = DateUtils.getRelativeTimeSpanString(lastUpdated.millis,
                System.currentTimeMillis(), DateUtils.SECOND_IN_MILLIS)
                .toString()
        lastUpdate.text = context.getString(R.string.last_updated, time)
    }
